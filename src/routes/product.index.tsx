  ...
  head: () => ({
    meta: [
      { title: "The Yuvro Labs Product — Engineering Hiring System" },
      {
        name: "description",
        content:
          "Engineering Simulations, Knowledge Assessments, Vitarka AI interviews, hiring intelligence, follow-ups and reports — one system to evaluate engineers.",
      },
      { property: "og:title", content: "The Yuvro Labs Product — Engineering Hiring System" },
      {
        property: "og:description",
        content: "Evaluate, verify, understand, follow up and hire engineers with evidence.",
      },
    ...
    <div className="min-h-screen text-[#0A0A0A] antialiased" style={{ background: "#FAFAF8" }}>
      ...
      <main className="mx-auto max-w-[1120px] px-6 py-16">
        ...
        <p className="mt-5 max-w-xl text-[15px] leading-relaxed text-[#6B6B6B]">
          Evaluate engineers through real work, verify the evidence behind every score, understand
          technical ability, follow up automatically and keep the hiring decision with your
          recruiters.
        </p>
          ...
          {productGroups.map((g) => (
            <section key={g.id}>
              <div className="flex items-baseline gap-3 border-b border-[#E8E6E1] pb-3">
                ...
                <span className="text-[11px] uppercase tracking-[0.12em] text-[#B4B0A8]">
                  {g.caption}
                </span>
              </div>
              <div className="grid gap-x-10 gap-y-7 pt-6 md:grid-cols-2">
                {g.items.map((it) => (
                  ...
                  >
                    <ProductGlyph
                      name={it.glyph}
                      className="mt-1 shrink-0 text-[#8A867E] group-hover:text-[#0A0A0A]"
                    />
                    <span>
                      <span className="block text-[15px] group-hover:underline">{it.title}</span>
                      <span className="mt-1 block max-w-md text-[13px] leading-relaxed text-[#6B6B6B]">
                        {it.description}
                      </span>
                    </span>
                  </Link>
