...
function CandidateWorkspace() {
  ...
  return (
    <div className="min-h-screen">
      {/* Top header */}
      <header className="bg-white border-b border-white/5 backdrop-blur">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-white/5 bg-gradient-to-br from-white/[0.04] to-transparent">
        ...
        <div className="grid grid-cols-3 gap-px bg-white/5 divide-x divide-black/10">
...
function EvaluationSummary({ candidate }: { candidate: { name: string; labsScore: number; assessmentScore: number; vitarkaScore: number; eci: number } }) {
  const data = buildEvaluationSummary(candidate);
  const { overall, verdict } = data;
  const verdictTone =
    verdict === "Shortlisted"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
        ...
        <p className="mt-3 text-[13px] leading-relaxed text-neutral-200">{data.overallText}</p>
        <div className="mt-5 grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            <div className="text-[10px] uppercase tracking-widest text-neutral-500">Strengths</div>
            <ul className="mt-1.5 space-y-1">
              {data.overallStrengths.map((s) => (
                <li key={s} className="flex gap-2 text-[12px] text-neutral-300">
                  <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-emerald-400" />
                  {s}
                </li>
...
function SubMetric({ label, value, onClick }: { label: string; value: number; onClick?: () => void }) {
  return (
    <button onClick={onClick} className="px-4 py-4 text-left transition">
      ...
      <div className="mt-1 text-[22px] font-medium text-black-300">
        {value}
