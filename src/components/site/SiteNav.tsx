    ...
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-neutral-200">
      <div className="mx-auto flex h-16 max-w-7xl items-center px-6">
        ...
        <nav className="ml-10 hidden md:flex items-center gap-7 text-sm text-[#6B6B6B]">
          <ProductMegaMenu />
          <Link to="/solutions" className="hover:text-[#0A0A0A] transition">
            Solutions
          </Link>
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          >
            <svg width="16" height="12" viewBox="0 0 16 12" fill="none">
              <path
                d={mobileOpen ? "M2 2l12 8M14 2 2 10" : "M1 1.5h14M1 6h14M1 10.5h14"}
                stroke="currentColor"
                strokeWidth="1.3"
                strokeLinecap="round"
              />
      ...
      {mobileOpen && (
        <div className="md:hidden max-h-[calc(100vh-4rem)] overflow-y-auto border-t border-[#E8E6E1] bg-white px-5 pb-8 pt-4">
          <p className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-[#8A867E]">
            Platform
          </p>
          <ProductMobileNav onNavigate={() => setMobileOpen(false)} />
          <div className="mt-5 flex flex-col gap-3 text-[15px]">
            <Link to="/solutions" onClick={() => setMobileOpen(false)} className="text-[#2A2A28]">
              Solutions
            </Link>
            <Link to="/pricing" onClick={() => setMobileOpen(false)} className="text-[#2A2A28]">
              Pricing
            </Link>
            <Link
              to="/recruiter-login"
